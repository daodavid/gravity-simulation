...
g.add_body(Body(5000, -1,-15 ,0.00001*np.cos(np.pi / 4), -0.001*np.cos(np.pi / 4),h=step))
...
for i in range(200):
    ...
    x2 = uniform(0,80)
    m  = uniform(100,500)
    g.add_body(Body(m ,x1,x2 ,1*np.cos(np.pi / 4), 1*np.cos(np.pi / 4),h=step))
    #g.add_body(Body(10000, x1+5,x2+6,1*np.cos(np.pi / 4), 1*np.cos(np.pi / 4),h=step))
...
g.add_body(Body(7000, 10,0 ,0.00001*np.cos(np.pi / 4), 0.01*np.cos(np.pi / 4),h=step))
...
anim.save('video/b12-4.mp4', writer=writer)
