...
for i in range(10):
    ...
    x2 =uniform(40,130)
    m = uniform(1000,2000)
    g.add_body(Body(m, x1,0.1 , 3, 2*np.cos(np.pi*(45/180)),h=step))
...
for i in range(10):
    ...
    x2 =uniform(-40,-130)
    m = uniform(1000,2000)
    g.add_body(Body(m, x1,-2 , 3,- 2*np.cos(np.pi*(45/180)),h=step))
...
anim.save('video/galaxy32134332.mp4', writer=writer)
