...
g.add_body(Body(50000, 50,90 , 0.001, 0.00001 ,h=step))
for i in range(300):
    v1 = uniform(50,70)
    v2 =uniform(50,70)
    x1 = uniform(40,60)
    x2 =uniform(40,80)
    m = uniform(500,2500)
    g.add_body(Body(m, x1,x2 , -v1*np.cos(np.pi*(v1/180))/50, -v2*np.cos(np.pi*(v1/180))/50 ,h=step))
...
anim.save('video/black_holdde_2.mp4', writer=writer)
