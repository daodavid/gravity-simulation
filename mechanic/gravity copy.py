...
class GravityField:
    ...
    def update_anim(self, i, fig, ax):
        ...
        ax.scatter(x_plot, y_plot ,color='skyblue')
        plt.plot(x, y ,color='skyblue',linewidth=linewidth/2) 

        #ax.scatter(tranformed_train_data[:,0], tranformed_train_data[:,1], tranformed_train_data[:,2])   
    ...
    def save_animation(self, path=None ,show_trajectory=True):
       ...
       # fig = plt.figure(figsize=(6, 6))
        ...
        anim = animation.FuncAnimation(plt.gcf(), self.update_anim, interval=1, fargs=(fig,ax,), frames=self.number_frames, blit=False)
...
field.run(5000, C = 0.001, number_frames=30,approx_error=2.5)
